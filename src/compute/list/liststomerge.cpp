...
ListsToMerge::ListsToMerge() {
	lists = NULL; edges = NULL; labels = NULL;
...
void ListsToMerge::mergeKLists() {
	if(numEdges) {
		...
		vertexid_t *newEdges; char *newLabels; 
		...
		if(numOfLists > 1) {
			newEdges = new vertexid_t[numEdges]; newLabels = new char[numEdges];
			// initial k-MinHeap
...
void ListsToMerge::addOneContainer() {
	if(numOfLists == 0) {
		...
		if(numOfLists >= capacityOfLists) {
			capacityOfLists *= 2;
			myrealloc(lists,numOfLists,capacityOfLists);
		}
	}
	// add one empty list
	++numOfLists;
